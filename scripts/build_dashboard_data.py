...
def build_technical_score_view() -> dict[str, Any]:
    ...
    items = [
        ...
        for name, score in sorted(collect_numeric_items(skill_score if isinstance(skill_score, dict) else {}), key=lambda x: x[0].lower())
    ]
    return {'items': items}
...
def build_skill_radar_view() -> dict[str, Any]:
    ...
    items = [
        ...
        for name, score in sorted(collect_numeric_items(skill_radar if isinstance(skill_radar, dict) else {}), key=lambda x: x[1], reverse=True)
    ]
    return {'items': items}
